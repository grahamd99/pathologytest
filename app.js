...
app.get("/", function (req, res) {
  fs.readFile(filePath, "utf8", (err, data) => {
    ...
    try {
      ...
      if (
        jsonParsed.resourceType === "Bundle" &&
        Array.isArray(jsonParsed.entry)
      ) {
        ...
        jsonParsed.entry.forEach((entry) => {
          if (
            ...
            specimenCollectedDateTime =
              entry.resource.collection.collectedDateTime;
            ...
            if (myList1.indexOf(thisCode) !== -1) {
              if (thisCode == "50121000237101") {
                ...
                console.log(
                  "The number of bodys structures is " + bodyStructureNum
                );
                global.obsBodySiteCode[i] = "";
                global.obsBodySiteDisplay[i] = "";
                var delim;
                ...
                for (var x in entry.resource.component) {
                  ...
                  global.obsBodySiteCode[i] =
                    global.obsBodySiteCode[i] +
                    entry.resource.component[x].valueCodeableConcept.coding[0]
                      .code +
                    delim;
                  global.obsBodySiteDisplay[i] =
                    global.obsBodySiteDisplay[i] +
            ...
            console.log(
              "Observation obsValueCode SNOMED CODE: " + global.obsValueCode[i]
            );
            console.log(
              "Observation obsValueDIsplay SNOMED Display: " +
                global.obsValueDisplay[i]
            );
            console.log(
              "Observation obsBodySiteCode SNOMED CODE: " +
                global.obsBodySiteCode[i]
            );
            console.log(
              "Observation obsBodySiteDisplay SNOMED Display: " +
                global.obsBodySiteDisplay[i]
            );
      ...
      res.render("Home", {
        ...
        messageHeaderProfile: messageHeaderProfile,
        messageHeaderCode: messageHeaderCode,
        messageHeaderDesc: messageHeaderDesc,
        patientProfile: patientProfile,
        patientNHS: patientNHS,
        serviceRequestProfile: serviceRequestProfile,
        serviceRequestCode: serviceRequestCode,
        serviceRequestDesc: serviceRequestDesc,
        specimenProfile: specimenProfile,
        specimenTypeCode: specimenTypeCode,
        specimenTypeDesc: specimenTypeDesc,
        specimenCollectedDateTime: specimenCollectedDateTime,
        specimenIdentifier: specimenIdentifier,
        diagnosticReportProfile: diagnosticReportProfile,
        diagnosticReportCode: diagnosticReportCode,
        diagnosticReportDesc: diagnosticReportDesc,
        practitionerProfile: practitionerProfile,
        practitionerAddress: practitionerAddress,
        practitionerName: practitionerName,
        obsProfile: global.obsProfile,
        obsCode: global.obsCode,
        obsDisplay: global.obsDisplay,
        obsBodySiteCode: global.obsBodySiteCode,
        obsBodySiteDisplay: global.obsBodySiteDisplay,
        obsValueCode: global.obsValueCode,
        obsValueDisplay: global.obsValueDisplay,
      });
    } catch (parseError) {
